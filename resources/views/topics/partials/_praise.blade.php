<div class="panel panel-default padding-md">
    ...
    <div class="panel-body text-center">
        ...
        <div class="btn-group">
            <?php
            $is_praised = Auth::user() && $topic->praises()->ByWhom(Auth::id())->WithType('praise')->exists();
            ?>
            @if ($is_praised)
                <a href="{{ route('topics.unpraise', $topic->id) }}" title="点赞"
                   class="btn btn-{{  $is_praised ? 'success' :'default' }}" >
                    <i class="fa fa-thumbs-o-up" aria-hidden="true"></i>
                    已赞过
                </a>
            @else
            <a href="{{ route('topics.praise', $topic->id) }}" title="点赞"
               <?php
               $is_praised = Auth::user() && $topic->praises()->ByWhom(Auth::id())->WithType('praise')->exists();
               ?>
               class="btn btn-{{  $is_praised ? 'success' :'default' }}" >
